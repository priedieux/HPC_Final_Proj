#!/bin/bash
# benchmark_mpi_local.sh - Local benchmark for MPI image processing
# Adjusted for typical desktop/laptop systems

echo "======================================"
echo "Image Processing MPI Local Benchmark"
echo "======================================"
...
# Adjust process counts based on available cores
# Use up to NUM_CORES, but reasonable test points
if [ $NUM_CORES -ge 16 ]; then
    processes=(1 2 4 8 12)
    echo "Using process counts: 1 2 4 8 12"
elif [ $NUM_CORES -ge 8 ]; then
    processes=(1 2 4 6 8)
    echo "Using process counts: 1 2 4 6 8"
else
    processes=(1 2 4)
    echo "Using process counts: 1 2 4"
fi
...
# Results file
results_file="results/benchmark_mpi_local_results.txt"
error_log="results/benchmark_mpi_errors.log"
echo "MPI Local Benchmark Results - $(date)" > $results_file
...
for size_name in "${!images[@]}"; do
    ...
    for filter in "${filters[@]}"; do
        ...
        for np in "${processes[@]}"; do
            ...
            # Add --oversubscribe flag to allow more processes than cores if needed
            output=$(mpirun --oversubscribe -np $np ./image_proc_mpi.exe "$input_file" "$output_file" "$filter" 2>&1)
            exit_code=$?

            if [ $exit_code -ne 0 ]; then
                echo "FAILED (exit code: $exit_code)"
                echo "  $np processes: FAILED (exit $exit_code)" >> $results_file
                echo "=== Error with $size_name / $filter / $np processes ===" >> $error_log
                ...
                if [ -z "$time" ]; then
                    echo "FAILED (no timing)"
                    echo "  $np processes: FAILED (no timing output)" >> $results_file
                    echo "=== No timing for $size_name / $filter / $np processes ===" >> $error_log
                    echo "$output" >> $error_log
                    echo "" >> $error_log
                else
                    echo "$time seconds"
...
try:
    with open('results/benchmark_mpi_local_results.txt', 'r') as f:
        content = f.read()
    ...
    with open('results/benchmark_mpi_local_results.txt', 'a') as f:
        f.write('\n\n')
        for line in output_lines:
            f.write(line + '\n')

    print("\nSpeedup metrics added to results/benchmark_mpi_local_results.txt")
